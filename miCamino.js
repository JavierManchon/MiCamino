...
for (let i = 0; i < miCamino.material.length; i++) {
  if (i === 0) {
    ...
    console.log(`y por último debo llevar es ${miCamino.material[i].nombre} que es ${miCamino.material[i].descripcion}`);
  }
}
...
for (let top of miCamino.cimas) {
  console.log(`para subir al ${top.nombre} debo subir ${top.altitud} metros y debo viajar a ${top.localizacion}`);
}
...
newPhrase = "";
for (let newString of miCamino.subiendoAlEverest[0][0].campoBase[0].campoI.campoII.campoIII.campoIV) {
  if (typeof newString === "string" && newPhrase === "") {
    newPhrase += newString;
